...
class InventoryManager:
    ...
    def gen_qr_code(self, serial:int, name:str, sap:int="-"):
        ...
        font = ImageFont.truetype("./arialbd.ttf", 70)  # You may need to install a font
        """if len(name) < 28:
            draw.text((width + 10, 40), name, font=font, fill="black")
            if sap != "-":
                draw.text((width + 10, 230), str(sap), font=font, fill="black")
        elif len(name) < 58:
            draw.text((width + 10, 40), name[:29], font=font, fill="black")
            draw.text((width + 10, 120), name[29:], font=font, fill="black")
            if sap != "-":
                draw.text((width + 10, 230), str(sap), font=font, fill="black")
        elif len(name) <= 87:
            draw.text((width + 10, 40), name[:29], font=font, fill="black")
            draw.text((width + 10, 120), name[29:58], font=font, fill="black")
            draw.text((width + 10, 180), name[58:], font=font, fill="black")
            if sap != "-":
                draw.text((width + 10, 230), str(sap), font=font, fill="black")"""

        wrapped_name = textwrap.wrap(name, width=28, max_lines=3)

        for i, line in enumerate(wrapped_name):
            y_position = 40 + (i * 80)  # Adjust y_position for each line
            draw.text((width + 10, y_position), line, font=font, fill="black")
        if sap != "-":
                draw.text((width + 10, 300), str(sap), font=font, fill="black")
        ...
        middle_img = Image.new("RGB", (2150, 593), "white")
        middle_img.paste(new_img, (367, 38))
        #middle_img.save(f"barcodes/{serial}_mid.png")
                        
        big_img = Image.new("RGB", (2150, 728), "white")
        big_img.paste(new_img, (367, 38))
        #big_img.save(f"barcodes/{serial}_big.png")

        pdf = FPDF()
        pdf.set_line_width(0.5)
        pdf.add_page()
        pdf.image(new_img, x=9, y=10.5, w=53, keep_aspect_ratio=True)
        pdf.rect(10, 10, 53, 15)
        pdf.image(middle_img, x=10, y=70, w=81, keep_aspect_ratio=True)
        pdf.rect(10, 70, 81, 23)
        pdf.image(big_img, x=10, y=150, w=81, keep_aspect_ratio=True)
        pdf.rect(10, 150, 81, 28)
    ...
    def download_selected_qr_codes(self, id_list):
        ...
        for id in self.running_data['id']:
                if os.path.exists(f"barcodes/{id}.pdf") != True:
                    print(str(id) + " : " + str(os.path.exists(f"barcodes/{id}.pdf")))
